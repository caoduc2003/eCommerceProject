<%@ taglib prefix="fmt" uri="http://java.sun.com/jsp/jstl/fmt" %>
    <%@ taglib prefix="c" uri="http://java.sun.com/jsp/jstl/core" %>
        <%@ page import="com.ecommerce.models.User" %>
            <%@ page import="com.ecommerce.models.CartItemDTO" %>
                <%@ page import="com.ecommerce.DAO.CartDAO" %>
                    <%-- Created by IntelliJ IDEA. User: brian Date: 10/12/2023 Time: 3:46 PM To change this template
                        ...
                        <%@ page contentType="text/html;charset=UTF-8" language="java" %>
                            <% if (session.getAttribute("user")==null) { response.sendRedirect("login.jsp"); } User
                                user=(User) session.getAttribute("user"); CartDAO cartDAO=new CartDAO(); %>
                                <c:set var="currUser" value="<%=user%>" />
                                <c:set var="cartCount"
                                    value="<%=cartDAO.getTotalCartItemsByUserID(user.getUserID())%>" />
                                <c:set var="cartTotal"
                                    value="<%=cartDAO.getTotalCartItemsPriceByUserID(user.getUserID())%>" />
                                <div
                                            ...
                                            <label tabindex="0" class="btn btn-ghost btn-circle">
                                                <div class="indicator">
                                                    ...
                                                    <span class="badge badge-sm indicator-item"
                                                        id="cart-badge">${cartCount}</span>
                                                </div>
                                            </label>
                                            <div tabindex="0"
                                                class="mt-3 z-[1] card card-compact dropdown-content w-52 bg-base-100 shadow">
                                                <div class="card-body">
                                                    <span class="font-bold text-lg" id="item-count">${cartCount}
                                                        Items</span>
